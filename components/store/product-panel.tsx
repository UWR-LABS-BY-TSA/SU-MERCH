...
export function ProductPanel() {
  ...
  return (
    <>
      ...
      {/* Centered viewer */}
      <aside
        className={`${open ? "pointer-events-auto" : "pointer-events-none"} fixed inset-0 z-30 flex items-center justify-center p-3 md:p-8`}
        aria-hidden={!open}
        role="dialog"
      >
...
function MicroficheCarrier({
  ...
  return (
    <div className="relative max-h-[94vh] w-full max-w-[640px] overflow-hidden rounded-sm border border-[#FFB100]/40 bg-gradient-to-b from-[#3a0410] via-[#2a0208] to-[#14100c] text-[#f1e6c8] shadow-[0_0_90px_rgba(147,0,11,0.35),0_20px_60px_rgba(0,0,0,0.75)] animate-microfiche-open overflow-y-auto">
      {/* ===== Burgundy header bar (gold trim, matches store palette) ===== */}
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-[#FFB100]/45 bg-gradient-to-r from-[#5a0810] via-[#7a0a14] to-[#5a0810] px-5 py-2.5 shadow-[inset_0_1px_0_rgba(255,177,0,0.25),inset_0_-1px_0_rgba(0,0,0,0.4)]">
        <span className="font-mono text-[9px] uppercase tracking-[0.35em] text-[#FFB100]">
          ⚜ Hall of Artifacts
        </span>
        ...
        <button
          onClick={closeProduct}
          aria-label="Close"
          className="flex h-6 w-6 items-center justify-center rounded-full text-[#FFB100] transition hover:bg-[#FFB100]/15"
        >
      ...
      {/* ===== Slide carrier ===== */}
      <div className="relative border-b border-[#FFB100]/25 bg-[#14100c] px-8 pb-4 pt-5 md:px-12">
        {/* Sprocket holes — top */}
        <SprocketRow />

        {/* The illuminated slide — dark charcoal with a warm gold backlight */}
        <div className="relative mt-3 h-56 overflow-hidden border border-[#FFB100]/50 bg-gradient-to-br from-[#3a0410] to-[#14100c] md:h-64">
          ...
          {product.imageUrl ? (
            ...
            <div className="relative z-10 flex h-full items-center justify-center">
              <div
                className="h-32 w-32 rounded-sm shadow-[0_12px_30px_rgba(0,0,0,0.5)]"
      ...
      <div className="px-6 py-5 md:px-8 md:py-6">
        {/* Title */}
        <h2 className="mb-5 font-serif text-2xl font-medium italic leading-tight text-[#ffd98a] md:text-3xl">
          {product.name}
        </h2>

        {/* Index card rows — type in sequentially */}
        <div className="mb-6 divide-y divide-[#FFB100]/15 border-y border-[#FFB100]/25">
          {indexRows.map((row, i) => (
            <div
              key={row.label}
              className="flex items-baseline gap-4 py-2 font-mono text-[10px] transition-all duration-300 md:text-[11px]"
              style={{
                opacity: visibleRows > i ? 1 : 0,
                transform: visibleRows > i ? "translateX(0)" : "translateX(-6px)",
              }}
            >
              <span className="w-24 shrink-0 uppercase tracking-[0.25em] text-[#c89232]">
                {row.label}
              </span>
              <span className="uppercase tracking-[0.15em] text-[#f1e6c8]">
                {row.value}
              </span>
            </div>
          ))}
              ...
              {product.sizes.map((s) => {
                ...
                return (
                  <button
                    key={s}
                    onClick={() => setSize(s)}
                    className={`min-w-[2.75rem] border px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-[0.2em] transition ${
                      active
        ...
        <div className="mb-5">
          ...
          <div className="inline-flex items-center gap-0 border border-[#FFB100]/25">
            <button
              ...
              className="flex h-8 w-8 items-center justify-center text-[#f1e6c8]/80 transition hover:bg-[#FFB100]/10"
              aria-label="Decrease"
            >
              <Minus className="h-3 w-3" />
            </button>
            <span className="flex h-8 w-10 items-center justify-center font-mono text-sm text-[#ffd98a]">
              {qty}
            </span>
            <button
        ...
        <button
          ...
          className="group relative flex w-full items-center justify-center gap-3 overflow-hidden rounded-sm border-2 border-[#FFB100]/60 bg-[#FFB100]/5 px-5 py-3.5 transition hover:border-[#FFB100] hover:bg-[#FFB100]/15"
        >
          <ShoppingBag className="h-3.5 w-3.5 text-[#ffd98a] transition group-hover:scale-110" />
